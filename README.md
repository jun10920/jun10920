...
안녕하세요! 저는 **정량적 성과**와 **깊이 있는 기술적 고민**으로 사용자 가치를 실현하는 백엔드 개발자입니다.

팀의 **마중물**이 되어 잠재력을 이끌어내는 것을 즐기며, **데이터 기반**의 의사결정으로 프로젝트의 성공을 이끌어냅니다.  
실제 서비스를 운영하며 **사용자 피드백**을 직접 반영하고, Query Plan 분석과 단계별 성능 최적화를 통해 최적의 해결책을 도출합니다.

> "좋은 서비스는 개발자가 아닌 사용자 관점에서 시작됩니다."

대규모 트래픽(10만/초)을 처리하고, 응답 시간을 획기적으로 개선(24초 → 0.1초)한 경험이 있습니다.  
이 과정에서 한정된 자원을 최대한 활용하여 비용 효율적인 아키텍처를 설계하는 것을 중요하게 생각합니다.

- 🎯 **고객 중심**: 실제 사용자의 피드백을 적극 수용하고, 데이터 기반으로 의사결정합니다.
- 👥 **팀 리딩**: 효율적인 협업 환경을 구축하고, 팀의 잠재력을 이끌어내는 **마중물** 역할을 합니다.
- 💡 **기술 깊이**: 단순 구현을 넘어 Query Plan 분석, 단계별 최적화 등 깊이 있는 고민을 통해 문제를 해결합니다.
...
🔗 [프로젝트 링크](https://github.com/now-here-5/.github) <br>
🔗 [블로그 링크: 실시간 데이터 기반 매칭 알고리즘 최적화: 동적 가중치 조정과 자동화](https://jun10920.tistory.com/38) <br>
🔗 [블로그 링크: 성능 테스트_성능 목표 잡기](https://jun10920.tistory.com/40)

---
### Filmeet: **디스커버리 커머스 기반 영화 콘텐츠 추천 서비스**
> **2024.11 ~ 2024.12**  
> **Stack**: Spring Boot, Java, MySQL, Redis, AWS (ECS, RDS, S3, ALB), Pinpoint, GitHub Actions  

"Filmeet"는 디스커버리 커머스 기반의 맞춤형 영화 추천 서비스를 제공하는 플랫폼으로,  
**AWS 기반 DevOps**와 **데이터베이스 성능 최적화**를 통해 안정적이고 신뢰할 수 있는 서비스를 구현했습니다.

> **"안정적이고 신뢰할 수 있는 시스템은 사용자 경험을 완성하는 핵심 기반이다."**

### 주요 기여
- **AWS 기반 CI/CD 및 DevOps 구축**:
  - CI/CD 파이프라인 자동화로 배포 시간을 **1시간에서 5분으로 90% 이상 단축**:
    - **AWS ECS(Fargate)**와 **ECR**을 활용하여 백엔드 자동 배포 구현.
    - **AWS Amplify**를 통해 프론트엔드 자동 배포 프로세스 구축.

- **데이터베이스 성능 최적화 및 병목 현상 해결**:
  - **대규모 데이터 환경에서의 쿼리 성능 최적화**:
    - **Pinpoint**와 **Query Plan 분석**을 통해 병목 구간을 발견하고 최적화.
    - **Generated Column**과 **Full Text Index**를 도입하여 쿼리 실행 시간을 **1297ms → 0.013ms**로 단축.
  - **DB 커넥션 풀 최적화**:
    - CS 원리를 기반으로 DB 커넥션 풀과 스레드 풀을 재설계하여 효율성을 극대화.
  - **결과**:
    - 평균 응답 시간을 **24초에서 0.1초로 단축(99.58% 개선)**.
    - 에러율을 **56%에서 0%로 감소(100% 개선)**.

- **게이미피케이션을 통한 개인 맞춤형 영화 추천 로직 구현**:
  - 사용자 참여를 유도하며 추천 로직의 효과를 극대화.

이 프로젝트를 통해 **AWS를 활용한 DevOps**, **데이터베이스 성능 최적화**,  
**개인화 로직 개발**의 주요 기술적 과제를 다루며, 실질적인 사용자 경험 개선에 기여했습니다.

🔗 [프로젝트 링크](https://github.com/LG-URECA-FINAL-TEAM8/filmeet-backend) <br>
🔗 [블로그 링크: 24초에서 0.1초로 - Query Plan 분석 / 커넥션 풀 조정 심화](https://jun10920.tistory.com/48) <br>
🔗 [블로그 링크: 야, 공짜로 할 수 있을 때 AWS 써라. (부제: DevOps는 정말 어려워)](https://jun10920.tistory.com/49)
...
- **사용자 중심 개발**: 실제 서비스 운영을 통해 **사용자 피드백**을 직접 반영하며, 개발자가 아닌 **사용자 관점**에서의 가치를 이해하게 되었습니다.

- **깊이 있는 성능 최적화**: 
  - **단계별 최적화**: HikariCP → Caffeine 캐싱 → 인덱싱 최적화로 성능 41% 개선
  - **데이터베이스**: Query Plan 분석으로 조회 시간 99.58% 단축 (24초 → 0.1초)
  - **인프라**: 한정된 자원에서 최대 성능을 끌어내는 아키텍처 설계 (10만 TPS 처리)

- **효과적인 팀 리딩**: 
  - 팀원들의 요구사항을 수용하고 일정을 관리하는 **마중물** 역할 수행
  - 효율적인 협업 프로세스와 인프라 구축으로 배포 시간 90% 단축 (1시간 → 5분)
---
